...
<div class="mb-8">
    ...
    <p class="text-[var(--color-neutral-700)]">Perbarui informasi fasilitas {{ $facility->name }}</p>
</div>
    ...
    <div class="grid lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <p class="text-sm text-[var(--color-neutral-700)] mt-2">Kosongkan jika tidak ingin mengubah gambar. Format: JPG, JPEG, PNG (Max 2MB)</p>
                
                <!-- Image Preview -->
                <div id="imagePreview" class="mt-6 hidden">
                    <p class="text-sm text-[var(--color-neutral-900)] mb-3">Preview Gambar Baru:</p>
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <textarea 
                    ...
                    rows="4"
                    class="w-full px-4 py-3 border border-[var(--color-neutral-300)] rounded-lg focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] focus:border-transparent"
                    placeholder="Masukkan deskripsi singkat tentang fasilitas...">{{ old('description', $facility->description) }}</textarea>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <div>
                    ...
                    <p class="text-sm text-[var(--color-neutral-700)] mt-1">Angka lebih kecil muncul lebih dulu</p>
                </div>
            </div>

            <!-- Current Status -->
            <div class="bg-white rounded-lg shadow-md p-6">
                <h4 class="mb-4">Status Saat Ini</h4>
                <div class="space-y-3">
                    <div class="flex items-center justify-between py-2 border-b border-[var(--color-neutral-200)]">
                        <span class="text-sm text-[var(--color-neutral-700)]">Status</span>
                        @if($facility->is_active)
                            <span class="text-sm bg-green-100 text-green-700 px-3 py-1 rounded-full">Aktif</span>
                        @else
                            <span class="text-sm bg-gray-100 text-gray-700 px-3 py-1 rounded-full">Tidak Aktif</span>
                        @endif
                    </div>
                    <div class="flex items-center justify-between py-2 border-b border-[var(--color-neutral-200)]">
                        <span class="text-sm text-[var(--color-neutral-700)]">Urutan</span>
                        <span class="text-sm text-[var(--color-neutral-900)] font-medium">#{{ $facility->order }}</span>
                    </div>
                    <div class="flex items-center justify-between py-2">
                        <span class="text-sm text-[var(--color-neutral-700)]">Dibuat</span>
                        <span class="text-sm text-[var(--color-neutral-900)]">{{ $facility->created_at->format('d M Y') }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-6">
                <div class="flex items-start gap-3 mb-3">
                    ...
                    <h4 class="text-blue-900">Tips Edit</h4>
                </div>
                <ul class="space-y-2 text-sm text-blue-800">
                    <li class="flex items-start gap-2">
                        <span>•</span>
                        <span>Gambar hanya diubah jika diperlukan</span>
                    </li>
                    <li class="flex items-start gap-2">
                        <span>•</span>
                        <span>Pastikan nama tetap deskriptif</span>
                    </li>
                    <li class="flex items-start gap-2">
                        <span>•</span>
                        <span>Perbarui deskripsi jika ada perubahan</span>
                    </li>
                    <li class="flex items-start gap-2">
                        <span>•</span>
                        <span>Atur urutan untuk tampilan optimal</span>
                    </li>
                </ul>
            </div>
            ...
            <div class="bg-red-50 border border-red-200 rounded-lg p-6">
                <div class="flex items-start gap-3 mb-3">
                    <i class="fa-solid fa-triangle-exclamation text-red-600 flex-shrink-0 mt-0.5"></i>
                    <h4 class="text-red-900">Zona Berbahaya</h4>
                </div>
                <p class="text-sm text-red-800 mb-4">Tindakan ini tidak dapat dibatalkan</p>
                <form action="{{ route('admin.facilities.destroy', $facility) }}" 
                      method="POST" 
                      onsubmit="return confirm('Yakin ingin menghapus fasilitas {{ $facility->name }}? Tindakan ini tidak dapat dibatalkan!');">
                    @csrf
                    @method('DELETE')
                    <button type="submit" 
